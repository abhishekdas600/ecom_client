...
const Cart:React.FC<CartInterface>=(props)=> {
  ...
  const calculateSubtotal = () => {
    ...
    return cart.reduce((total:number, product: CartLayoutInteface) => total + ((product.quantity as number )* product.item.price), 0);
  };

  return (
    <div>
      <button onClick={handleClick}> <FiShoppingCart /></button>
                                  ...
                                  <div className="ml-4 flex flex-1 flex-col">
                                    <div>
                                      <div className="flex justify-between text-base font-medium text-gray-900">
                                        <h3>
                                          <a href={`/productPage/${product.item.id}`}>{product.item.title}</a>
                                        </h3>
                                        <p className="ml-4">${(product.quantity as number)*(product.item.price)}</p>
                                      </div>
                                    </div>
                                    <div className="flex flex-1 items-end justify-between text-sm">
                                      ...
                                      <div className="flex">
                                        <button
                                          onClick={()=> handleRemoveItem(product.item.id)}
                                          type="button"
